...
if __name__ == '__main__':
    ...
    print("Reading docking templates ...")
    docking_templates = pd.read_csv("../data/docking_templates.csv", index_col=0)
    docking_templates = docking_templates.to_dict("index")

    print("Getting list of files ...")
    path_list = [path.stem for path in Path("../data/posit_template").glob("*.sdf")]
    ...
    for _, representative in kinase_conformation_representatives.iterrows():
        ...
        for _, ligand_details in kinase_conformation_group.iterrows():
            ...
            docking_template = docking_templates[ligand_name.replace("-", "_")]
            ligand.docking_template_pdb_id = docking_template["docking_template_pdb_id"]
            ligand.docking_template_expo_id = docking_template["docking_template_expo_id"]
            ligand.docking_template_chain_id = docking_template["docking_template_chain_id"]
    ...
    featurizer = OEPositDockingFeaturizer(
        ...
        output_dir="../data/posit_template",
        n_processes=67
    )
